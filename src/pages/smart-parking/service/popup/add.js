...
const Add = ({ open, onClose, id, setReload }) => {
  ...
  const handleDailyPrice = (index, event) => {
    const newRows = [...servicePrice]
    newRows[index].dailyPrice = parseInt(event.target.value)
    setServicePrice(newRows)
  }

  const handleWeekendPrice = (index, event) => {
    const newRows = [...servicePrice]
    newRows[index].weekendPrice = parseInt(event.target.value)
    setServicePrice(newRows)
  }

  const handleHolidayPrice = (index, event) => {
    const newRows = [...servicePrice]
    newRows[index].holidayPrice = parseInt(event.target.value)
    setServicePrice(newRows)
  }

  const handleTimeFree = (index, event) => {
    const newRows = [...servicePrice]
    newRows[index].timeFree = parseInt(event.target.value)
    setServicePrice(newRows)
  }

  const handleWeight = (index, event) => {
    const newRows = [...servicePrice]
    newRows[index].weight = parseInt(event.target.value)
    setServicePrice(newRows)
  }
  ...
  const onSubmit = values => {
    ...
    postApi(`https://dev-ivi.basesystem.one/camnet/camnet_parking/api/v0/service/parking/create`, { ...params })
      .then(() => {
                    ...
                    {parking.map((row, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          <Autocomplete
                            options={parkingList}
                            getOptionLabel={option => option.codeParking}
