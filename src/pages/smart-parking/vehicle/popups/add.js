...
const Add = ({ open, setReload, onClose }) => {
  ...
  const fetchParentData = async parentId => {
    try {
      const response = await axios.get(`https://dev-ivi.basesystem.one/smc/infrares/api/v0/regions/${parentId}`, config)
  ...
  const fetchBrandData = async () => {
    try {
      const response = await axios.get(
        `https://dev-ivi.basesystem.one/smc/infrares/api/v0/regions/code?Code=honda&sort=%2Bcreated_at&page=1`,
        config
      )
