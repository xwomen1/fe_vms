  ...
  const downloadFileExample = async () => {
    try {
      ...
      if (response && response.data) {
        ...
        a.href = url
        a.download = 'guest_template.xlsx'
      ...
      .then(rs => {
        if (rs.data && Array.isArray(rs.data)) {
          if (rs.data.length > 0) {
            ...
            if (errors.length > 0) {
              toast.error(`Some information is invalid, see the result file for details.`)
              ...
              toast.success(`Successfully added ${success.length} files.`)
            }
          } else {
