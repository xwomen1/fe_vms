...
import AddGuest from '../../scheduling/popups/addGuest'
import EditGuest from '../../scheduling/popups/editGuest'
import ImportGuest from '../../scheduling/popups/importGuest'
...
const Register = () => {
  ...
  const id = router.query.register
  const [data, setData] = useState([])
  const token = localStorage.getItem(authConfig.storageTokenKeyName)
  const [startTime, setStartTime] = useState(0) // Khởi tạo giá trị mặc định
  const [endTime, setEndTime] = useState(0) // Khởi tạo giá trị mặc định
  ...
  const fetchDataList = async () => {
    ...
    try {
      ...
      reset({
        ...defaultValues,
        area: { id: data.areaId, name: data.areaName },
        group: data.groupId,
      ...
      setStartTime(data.startTimeInMinute / 60)
      setEndTime(data.endTimeInMinute / 60)
  ...
  const isDisableDayOfWeekOption = option => {
    ...
    return !eachDayOfRange.some(day => DAYS_OF_WEEK[day.getDay()] == option.value)
  }
  ...
  const fetchUnitGroup = async () => {
    try {
      const response = await getApi(`https://dev-ivi.basesystem.one/smc/infrares/api/v0/regions`)
  ...
  const fetchAreaGroup = async () => {
    try {
      const response = await getApi(`https://dev-ivi.basesystem.one/smc/infrares/api/v0/regions/adults`)
  ...
  const fetchApproverGroup = async () => {
    try {
      const response = await getApi(
        `https://sbs.basesystem.one/ivis/iam/api/v0/users/search?limit=50&page=1&keyword=&isLeader=true`
      )
  ...
  const onSubmit = async values => {
    console.log(values, 'value,vlaue')
    console.log(approvalPersons, 'approvalPersons')

    const finalStartTime = startTime !== null ? startTime : values.startTimeInMinute / 60
    const finalEndTime = endTime !== null ? endTime : values.endTimeInMinute / 60
    ...
    if (check72Hours) {
      const params = {
        ...
        groupId: typeof values?.group === 'object' ? values?.group?.id : values?.group,
        startTimeInMinute: finalStartTime * 60,
        endTimeInMinute: finalEndTime * 60,
  ...
  const handleAddRowGuests = data => {
    ...
    const uniqueArray = arr.filter(
      (item, index, self) =>
        index === self.findIndex(t => t.accessCode === item.accessCode && t.identityNumber === item.identityNumber)
    )
  ...
  const handleSuccessImport = ret => {
    if (ret) {
      ...
      if (newGuest) {
        const flagRepresentation = newGuest.findIndex(g => g.isRepresentation)
  ...
  const vehicleCol = [
    {
      ...
      renderCell: (value, index) => (
        <CustomTextField
          value={value}
          onChange={event => handleChangeVehicleRow(index, event, 'vehicleName')}
          fullWidth
        />
      )
    },
    {
      ...
      renderCell: (value, index) => (
        <CustomTextField
          value={value}
          onChange={event => handleChangeVehicleRow(index, event, 'numberPlate')}
          fullWidth
        />
      )
    },
        ...
        return (
          <Autocomplete
            ...
            getOptionLabel={option => {
              ...
              return option?.fullName || guest?.fullName
            }}
  ...
  const handleChangeApproverRow = (index, event) => {
    ...
    const currentUser = newRowApprovers[index]?.users || null
    newRowApprovers[index].users = event || currentUser
    newRowApprovers[index].positionName =
      event?.userGroup[0]?.positionName || currentUser?.userGroup[0]?.positionName || ''
    newRowApprovers[index].mainGroupName = event?.mainGroupName || currentUser?.mainGroupName || ''

    newRowApprovalPersons[index] = event || currentUser
  ...
  const approverCol = [
    {
      ...
      renderCell: (value, index) => {
        ...
        const matchedUser = users.find(user => user.userId === value)
        const currentUser = approvers[index]?.users || matchedUser || null
        ...
        return (
          <Autocomplete
            ...
            onChange={(event, newValue) => {
              ...
              handleChangeApproverRow(index, newValue)
            }}
          ...
          {guests.length > 0 ? (
            guests.map((row, index) => (
              <TableRow hover tabIndex={-1} key={index}>
                ...
                <TableCell align='center'>
                  <Checkbox checked={row?.isRepresentation} onChange={() => handleCheckboxChange(index)} />
                </TableCell>
                <TableCell align='center'>
                  <IconButton
                    size='small'
            ...
            <TableRow>
              <TableCell colSpan={guestsCol.length + 3} align='center'>
                No guests available
              </TableCell>
            </TableRow>
          )}
      ...
      <Table stickyHeader aria-label='sticky table' sx={{ overflow: 'auto' }}>
        <TableHead>
          <TableRow>
            ...
            {vehicleCol.map(column => (
              <TableCell key={column.dataField} align={column.align} sx={{ minWidth: column.minWidth }}>
                {column.label}
              </TableCell>
            ))}
            <TableCell align='center'>Actions</TableCell>
        ...
        <TableBody>
          {vehicles.map((row, index) => (
            <TableRow hover tabIndex={-1} key={index}>
              <TableCell>{index + 1}</TableCell>
              {vehicleCol.map(({ dataField, renderCell, align }) => {
                const value = row[dataField]
              ...
              <TableCell align='center'>
                <IconButton
                  ...
                  onClick={() => {
                    setIsOpenDel(true)
                    setRowDel({ index, row, type: 'vehicle' })
                  }}
                >
                  <Icon icon='tabler:trash' />
                </IconButton>
          ...
          <Box sx={{ mb: 4, textAlign: 'center' }}>
            ...
            <Typography sx={{ color: 'text.secondary' }}>
              {isDelGuest && rowDel.row?.isRepresentation
                ? `Guest ${rowDel.row?.fullName} is currently a representative guest. 
  ...
  return (
    <>
      <Box sx={{ marginBottom: 3, display: 'flex', justifyContent: 'space-between' }}>
        ...
        <Box>
          <Button variant='contained' color='secondary' component={Link} href={`/pages/scheduling/detail/${id}`}>
            Cancel
          </Button>
          <Button
            type='submit'
      ...
      <form>
                          ...
                          onChange={e => {
                            ...
                            if (repeatType == 'WEEKLY') {
                              if (isAfter(startOfDay(e), startOfDay(endDate))) {
                                ...
                                const days = uniq(eachDayOfRange.map(d => DAYS_OF_WEEK[d.getDay()]))
                                setDayOfWeeks(days)
                              }
                            }
                          }}
              ...
              <Grid item xs={4}>
                ...
                <Slider
                  ...
                  onChange={(e, newVal) => {
                    ...
                    setEndTime(newVal[1])
                    setValue('startTimeInMinute', newVal[0] * 60)
                    setValue('endTimeInMinute', newVal[1] * 60)
                  }}
        ...
        <Card sx={{ margin: 2 }}>
            ...
            action={
              <Grid container spacing={2}>
                ...
                {isOpenAddGuest && (
                  <Grid item xs={12}>
                    <AddGuest
                      onClose={() => setIsOpenAddGuest(false)}
                      callback={handleAddRowGuests}
          ...
          />
          <CardContent>{TableGuest()}</CardContent>
        </Card>
        <Card sx={{ margin: 2 }}>
          <CardHeader
          ...
          />
          <CardContent>{TableVehicle()}</CardContent>
        </Card>
        <Card sx={{ margin: 2 }}>
          <CardHeader
          ...
          />
          <CardContent>{TableApprover()}</CardContent>
        </Card>
      </form>
      {isOpenEditGuest && (
        <EditGuest
          show={isOpenEditGuest}
          onClose={() => setIsOpenEditGuest(false)}
          data={guestEdit}
          callback={handleUpdateGuests}
        />
      )}
      {isOpenDel && DeleteView()}
      {isOpenImportGuest && (
        <ImportGuest
          show={isOpenImportGuest}
          onClose={() => setIsOpenImportGuest(false)}
