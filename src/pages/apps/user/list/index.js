...
import CheckboxTreeView from '../checkboxes';
...
const handleGroupSelect = selectedGroups => {
  // Logic to handle selected groups
...
const UserList = ({ apiData }) => {
  ...
  const token = localStorage.getItem(authConfig.storageTokenKeyName);
    ...
    // useEffect(() => {
    //   // Tính toán số trang dựa trên số lượng dòng trả về từ API và số dòng bạn muốn hiển thị trên mỗi trang
    //   const calculateTotalPages = () => {
    //     const totalRows = userData.length; // Số dòng trả về từ API
    //     const rowsPerPage = pageSize; // Số dòng bạn muốn hiển thị trên mỗi trang
    //     const pages = Math.ceil(totalRows / rowsPerPage);
    //     return pages;
    //   };
    
    //   // Cập nhật số trang
    //   setTotal(calculateTotalPages());
    // }, [userData, pageSize]);
    ...
    const handleSelectPageSize = (size) => {
      setPageSize(size);
      setPage(1); 
      handleCloseMenu(); // Đóng dropdown/menu sau khi chọn
    };
  ...
  const fetchDataWithPageSize = useCallback(async () => {
    try {
  ...
  }, [pageSize,page,selectedGroup, total]); // Theo dõi sự thay đổi của pageSize để gọi lại hàm này
  useEffect(() => {
 ...
 console.log(total, 'totalpage')
  ...
  return (
   ...
   <Grid container spacing={2}>
   <Grid > <IconButton onClick={handleOpenMenu}>
